...
class AuthController extends BaseController
{
	 ...
	 * @var array
	 */
	public $freeAccessActions = ['login', 'logout', 'confirm-registration-email','change-own-password','tables','select-table'];
	...
	public function actionLogin($table=NULL)
	{
	//   if(is_null($table)){
	...
	//   }
			...
			if ($model->load(Yii::$app->request->post()) )
		{
			...
			foreach($users as $user){
			  if(Yii::$app->getSecurity()->validatePassword($post["LoginForm"]["password"],$user['password_hash'])){
				   array_push( $matchingHash,$user['password_hash']);
			  }
			}
		   if( count($matchingHash) < 1){
			Yii::$app->session->setFlash('error', "");
		    return $this->redirect(['login']);
		   }
		  $user=$USERMODEL->find()->where(['password_hash'=>$matchingHash[0]])->one();
		...
		if ( $model->load(Yii::$app->request->post()) AND $model->login() )
		{
		   ...
		   if(Yii::$app->getUser()->identity->firstlogin==1){
			 ...
			 //return $this->redirect(['/site/table','id'=>$table]);
			 if(Yii::$app->getUser()->identity->superadmin==1){
				return $this->redirect(['/site/index','id'=>1]);
			 }else{
				return $this->redirect(['tables']);
			 }
